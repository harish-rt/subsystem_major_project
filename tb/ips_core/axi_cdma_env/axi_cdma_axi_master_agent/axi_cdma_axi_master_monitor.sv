...
/*                                                                        */
/* AXI_INTERCONNECT_DESIGN/AXI_TB/axi_cdma_axi_master_monitor.sv                       */
/*                                                                        */
/* RAITON CONFIDENTIAL                                                    */
/*                                                                        */
...
class axi_cdma_axi_master_monitor extends uvm_monitor;

   `uvm_component_utils (axi_cdma_axi_master_monitor)
   uvm_analysis_port #(axi_cdma_axi_master_seq_item)   mon_ap;
   virtual axi_cdma_axi_master_intf.MON_MOD_master     master_mon_intf;
   mailbox #(axi_cdma_axi_master_seq_item)  write_address_mbx ,write_data_mbx;      //to capture transactions on write address channel // Preserve ordering
   mailbox #(axi_cdma_axi_master_seq_item) wresp_array [id_t];    //associative array of mailboxes. Will hold packets waiting for write response.Mailboxes will preserve ordering based on ID.
   mailbox #(axi_cdma_axi_master_seq_item) read_address_array [id_t];
   mailbox #(axi_cdma_axi_master_seq_item) read_data_array[id_t];

   function new (string name = "axi_cdma_axi_master_monitor" , uvm_component parent);
   ...
   extern task  trigger_put_response();

endclass :axi_cdma_axi_master_monitor

   function void axi_cdma_axi_master_monitor :: build_phase (uvm_phase phase);
     super.build_phase (phase);
     mon_ap = new ("mon_ap",this);
     `uvm_info (get_full_name() , phase.get_name() , UVM_MEDIUM)
   endfunction : build_phase

task axi_cdma_axi_master_monitor :: main_phase (uvm_phase phase);
 `uvm_info (get_full_name() , phase.get_name() , UVM_MEDIUM)
 fork
  capture_reset();
...
task  axi_cdma_axi_master_monitor :: capture_write_address();
 axi_cdma_axi_master_seq_item     pkt;
 write_address_mbx = new ();
 forever begin
    `uvm_info("axi_cdma_axi_master_monitor :: capture_write_address","Triggred",UVM_LOW);
    ...
    `uvm_info("axi_cdma_axi_master_monitor",$sformatf("awid from if = %0d",master_mon_intf.mas_mon_cb.awid),UVM_LOW); 
    ...
    pkt.operation = WRITE;
    //`uvm_info("axi_cdma_axi_master_monitor :: capture_write_address","captured address pkt put to write_address_mbx",UVM_LOW);
    write_address_mbx.put(pkt);
    @(master_mon_intf.mas_mon_cb); //wait for a clk
  end
endtask

task  axi_cdma_axi_master_monitor ::  capture_write_data();
 axi_cdma_axi_master_seq_item     pkt;
 int i;
 bit last;
 write_data_mbx =    new ();
 forever begin
    `uvm_info("axi_cdma_axi_master_monitor :: capture_write_data","Triggred",UVM_LOW);
    pkt = axi_cdma_axi_master_seq_item :: type_id :: create("pkt");
    ...
    do begin
      ...
      //`uvm_info("axi_cdma_axi_master_monitor :: capture_write_data","Inside dowhile loop",UVM_LOW);
      ...
      i=i+1;
      if(last==1) write_data_mbx.put(pkt);
      ...
      //`uvm_info("axi_cdma_axi_master_monitor :: capture_write_data","captured data pkt put to write_data_mbx",UVM_LOW);
 end
endtask

task  axi_cdma_axi_master_monitor ::  capture_write_response();
 axi_cdma_axi_master_seq_item     pkt;
 forever begin
    `uvm_info("axi_cdma_axi_master_monitor :: capture_write_response","Triggred",UVM_LOW);
    wait( master_mon_intf.mas_mon_cb.bready==1 && master_mon_intf.mas_mon_cb.bvalid==1 && master_mon_intf.areset_n==1);
       trigger_put_response();
    ...
    `uvm_info("axi_cdma_axi_master_monitor :: capture_write_response","Sending pkt to SB",UVM_LOW);
...
task  axi_cdma_axi_master_monitor :: capture_read_address();
 axi_cdma_axi_master_seq_item     pkt;
 forever begin
    `uvm_info("axi_cdma_axi_master_monitor :: capture_read_address","Triggred",UVM_LOW);
    pkt = axi_cdma_axi_master_seq_item :: type_id :: create("pkt");
    ...
    pkt.operation = READ;
    //`uvm_info("axi_cdma_axi_master_monitor :: capture_read_address","captured address pkt put to read_address_array",UVM_LOW);
...
task  axi_cdma_axi_master_monitor ::  capture_read_data();
 axi_cdma_axi_master_seq_item     pkt, pkt2sb;
 int i,no_of_beats;
 bit last;
 forever begin
    `uvm_info("axi_cdma_axi_master_monitor :: capture_read_data","Triggred",UVM_LOW);
    i = 0;
    do begin
      pkt = axi_cdma_axi_master_seq_item :: type_id :: create("pkt");
    ...
    trigger_put_response();
      ...
      pkt2sb = axi_cdma_axi_master_seq_item :: type_id :: create("pkt2sb");
   ...
   for(i=0 ; i<no_of_beats; i++)begin         //merges all beats with same rid to pkt having address info
    ...
    `uvm_info("axi_cdma_axi_master_monitor :: capture_read_data","Sending pkt to SB",UVM_LOW);
...
task axi_cdma_axi_master_monitor :: merge_write_info();
axi_cdma_axi_master_seq_item addr_pkt,data_pkt,merged_pkt;
int x ,no_addr, no_data; //indicates number of completed write transactions waiting for response.
  `uvm_info("axi_cdma_axi_master_monitor :: merge_write_info","Triggred",UVM_LOW);
  no_addr = write_address_mbx.num();
  no_data = write_data_mbx.num();
  x = (no_addr<no_data)? no_addr :no_data;
  //`uvm_info("axi_cdma_axi_master_monitor :: merge_write_info",$sformatf("before get pkt comparing no_of_elements  addr= %d, data=%d ,x=%d",no_addr,no_data,x),UVM_LOW);
  repeat(x)begin
  merged_pkt = axi_cdma_axi_master_seq_item :: type_id :: create("merged_pkt");
  ...
  // `uvm_info("axi_cdma_axi_master_monitor :: merge_write_info",$sformatf("putting merged pkt to wresp_array awid = %b",merged_pkt.awid),UVM_LOW);
...
task axi_cdma_axi_master_monitor :: capture_reset();
 axi_cdma_axi_master_seq_item     rst_pkt;
 `uvm_info("axi_cdma_axi_master_monitor :: capture_reset","Triggred",UVM_LOW);
 fork
  forever begin //reset deasserted
    @(posedge master_mon_intf.areset_n) ;
    rst_pkt = axi_cdma_axi_master_seq_item :: type_id :: create("rst_pkt");
    rst_pkt.reset_op = RESET_DEASSERTED;
    rst_pkt.reset_deasserted = $realtime();
    `uvm_info("axi_cdma_axi_master_monitor :: capture_reset","reset_deasserted_pkt to SB",UVM_LOW);
  ...
  forever begin //reset asserted
    ...
    rst_pkt.reset_deasserted = $realtime();
    `uvm_info("axi_cdma_axi_master_monitor :: capture_reset","reset_asserted_pkt to SB",UVM_LOW);
    mon_ap.write(rst_pkt);
  end
join
endtask
...
task axi_cdma_axi_master_monitor :: trigger_put_response();
     uvm_event put_resp_trigger_event;
     uvm_event_pool m_event_pool;
    `uvm_info("axi_cdma_axi_master_monitor :: trigger_put_response","trigger_put_response called",UVM_NONE);
     m_event_pool = uvm_event_pool::get_global_pool;
     put_resp_trigger_event = m_event_pool.get("put_response_trigger");
     put_resp_trigger_event.trigger(put_resp_trigger_event);

    `uvm_info("axi_cdma_axi_master_monitor :: trigger_put_response","put_resp_trigger_event triggered",UVM_LOW);
 
endtask : trigger_put_response
