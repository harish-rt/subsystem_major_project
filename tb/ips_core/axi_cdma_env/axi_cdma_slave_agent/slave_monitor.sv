...
task  slave_monitor :: capture_write_address();
 ...
 forever begin
    ...
    pkt.operation = WRITE;
    `uvm_info("writing_to_resp_ap",pkt.sprint(),UVM_DEBUG)
    resp_ap.write(pkt); //write pkt to sequencer for response
    `uvm_info("after_writing_to_resp",pkt.sprint(),UVM_DEBUG)
    ...
    `uvm_info("slave_monitor::capture_write_address",pkt.sprint(),UVM_DEBUG)
...
task  slave_monitor ::  capture_write_data();
 ...
 forever begin
    ...
    do begin
      ...
      //`uvm_info("slave_monitor::capture_write_data",pkt.sprint(),UVM_LOW)
...
task  slave_monitor ::  capture_write_response();
 ...
 forever begin
    ...
    $display("--------------------after wait in write response phase--------------");
    ...
    if(wresp_array.exists(slave_mon_intf.slv_mon_cb.bid)) begin    //to check if captured bid matches with any transaction waiting for response.
       ...
       //`uvm_info("slave_monitor::write_response_phase",pkt.sprint(),UVM_LOW)
    end
    else `uvm_error("slave_monitor :: capture_write_response",$sformatf("unexpected write response, BID not found bid= %b",slave_mon_intf.slv_mon_cb.bid))
    @(slave_mon_intf.slv_mon_cb); //wait till next clk posedge
 end
endtask
...
task  slave_monitor :: capture_read_address();
 ...
 forever begin
    ...
    /*$display("captured araddr signal from slace monitor =%0h",slave_mon_intf.slv_mon_cb.araddr);
    $display("captured data to packet =%0h",pkt.araddr);
    $display("Width of interface araddr = %0d",$bits(slave_mon_intf.slv_mon_cb.araddr));
    $display("Width of pkt.araddr = %0d",$bits(pkt.araddr));*/
    ...
    pkt.operation = READ;
    resp_ap.write(pkt); //write pkt to sequencer for response
    `uvm_info("putting packet to analysis fifo in read address phase",pkt.sprint(),UVM_DEBUG)
...
task  slave_monitor ::  capture_read_data();
 ...
 forever begin
    ...
    `uvm_info("before merging read data",pkt.sprint(),UVM_LOW)
   ...
   for(i=0 ; i<no_of_beats; i++)begin         //merges all beats with same rid to pkt having address info
    ...
    `uvm_info("slave_monitor_after_writing_to_scoreboard",pkt2sb.sprint(),UVM_LOW)
 end
endtask
