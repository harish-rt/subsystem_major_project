...
/*                                                                        */
/* AXI_INTERCONNECT_DESIGN/AXI_TB/slave_driver.sv                         */
/*                                                                        */
/* RAITON CONFIDENTIAL                                                    */
/*                                                                        */
...
task slave_driver :: get_packet(); // to get new packets from seq and populate respective queues with relevent data.
   ...
   forever begin
    ...
    `uvm_info(get_full_name(),pkt.sprint(),UVM_DEBUG)
      wdata_mbx.put(pkt);
    `uvm_info(get_full_name(),pkt.sprint(),UVM_DEBUG)
      waddress_mbx.put(pkt);
      `uvm_info("slave_driver_after_putting_packet_to_waddress_mailbox",pkt.sprint(),UVM_DEBUG)
    end
...
`uvm_info(get_full_name(),"........slave_driver.......drive_write_add task triggred",  UVM_LOW);
 forever begin
   `uvm_info("slave_driver_write_addr","waiting for address packet",UVM_DEBUG)
   waddress_mbx.get(pkt);
   `uvm_info("slave_driver_write_address_phase",pkt.sprint(),UVM_LOW)
...
`uvm_info(get_full_name(),"........slave_driver.......drive_read_add task triggred",  UVM_LOW);
 ...
 `uvm_info("SLAVE DRIVER::AFTER GETTING PACKET",pkt.sprint(),UVM_LOW)
...
slave_seq_item pkt;
  ...
  forever begin
  `uvm_info("slave_driver_before_get","waiting for packet in the write response phase",UVM_LOW)
   wresponse_mbx.get(pkt);
   `uvm_info("slave_driver_driver",pkt.sprint(),UVM_LOW)
...
`uvm_info(get_full_name(),"........slave_driver.......drive_write_data task triggred",  UVM_LOW);
 forever begin
   `uvm_info("slave_driver_drive_write_data","before getting packet",UVM_DEBUG)
   wdata_mbx.get(pkt);
   `uvm_info("slave_driver_after getting_write_data_packet",pkt.sprint(),UVM_LOW)
  ...
  // wait(slave_drv_intf.slv_drv_cb.wvalid ==0);
   ...
   `uvm_info("slave_driver_write_data_before_putting_into_wresponse",pkt.sprint(),UVM_LOW)
  wresponse_mbx.put(pkt);
  `uvm_info("slave_driver_write_data_after_putting_into_wresponse",pkt.sprint(),UVM_LOW)
 end
endtask
