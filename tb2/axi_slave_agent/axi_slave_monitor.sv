...
/*                                                                        */
/* AXI_INTERCONNECT_DESIGN/AXI_TB/axi_slave_monitor.sv                        */
/*                                                                        */
/* RAITON CONFIDENTIAL                                                    */
/*                                                                        */
...
class axi_slave_monitor extends uvm_monitor;
   ...
   //uvm_analysis_port #(axi_slave_seq_item)   mon_ap;
   //uvm_analysis_port #(axi_slave_seq_item)   resp_ap;
   uvm_analysis_port #(axi_slave_seq_item)   addr_ph_port;

   
   virtual axi_slave_intf.MON_MOD_slave      slave_mon_intf;
   mailbox #(axi_slave_seq_item)  write_address_mbx ,write_data_mbx;      //to capture transactions on write address channel // Preserve ordering
   //mailbox #(axi_slave_seq_item) wresp_array [id_t];    //associative array of mailboxes. Will hold packets waiting for write response.Mailboxes will preserve ordering based on ID.
   mailbox #(axi_slave_seq_item) read_address_mbx;
   mailbox #(axi_slave_seq_item) read_data_mbx;

   int cdma_base = 32'h7000_0000;
   int intc_base = 32'h7000_1000;
   int rdaddr[$];
...
task axi_slave_monitor :: main_phase (uvm_phase phase);
 `uvm_info (get_full_name() , phase.get_name() , UVM_MEDIUM)
...
task  axi_slave_monitor :: capture_write_address();
 axi_slave_seq_item     pkt;
 write_address_mbx = new ();
 forever begin
    `uvm_info("axi_slave_monitor :: capture_write_address",$sformatf("Triggred%d,-%d",slave_mon_intf.slv_mon_cb.awvalid,slave_mon_intf.areset_n),UVM_LOW);
    pkt = axi_slave_seq_item :: type_id :: create("pkt");
    @(slave_mon_intf.slv_mon_cb); //wait till next clk posedge
    wait(slave_mon_intf.slv_mon_cb.awready && slave_mon_intf.slv_mon_cb.awvalid && slave_mon_intf.areset_n==1); //wait for valid
    pkt.awaddr   = slave_mon_intf.slv_mon_cb.awaddr;
    pkt.operation = WRITE;
    `uvm_info("axi_slave_monitor :: capture_write_address","before write",UVM_LOW);
    //addr_ph_port.write(pkt); //write pkt to sequencer for response
    `uvm_info("axi_slave_monitor :: capture_write_address","after write",UVM_LOW);
    ...
    //`uvm_info("axi_slave_monitor :: capture_write_address","captured address pkt put to write_address_mbx",UVM_LOW);
    write_address_mbx.put(pkt);
  end
endtask

task  axi_slave_monitor ::  capture_write_data();
 axi_slave_seq_item     pkt;
 //int i;
 //bit last;
 write_data_mbx =    new ();
 forever begin
    `uvm_info("axi_slave_monitor :: capture_write_data","Triggred",UVM_LOW);
    pkt = axi_slave_seq_item :: type_id :: create("pkt");
    ...
    pkt.wdata_hndshk = $realtime();  //capturing timestamp for handshake.
    pkt.wdata    = slave_mon_intf.slv_mon_cb.wdata;
    pkt.wstrobe  = slave_mon_intf.slv_mon_cb.wstrobe;

    //last = slave_mon_intf.slv_mon_cb.wlast;
    //i=i+1;
    //end while( last==0); //keeps sampling till last indicates end of data phase.
    //`uvm_info("axi_slave_monitor :: capture_write_data","captured data pkt put to write_data_mbx",UVM_LOW);
    write_data_mbx.put(pkt);
 end
endtask

task  axi_slave_monitor ::  capture_write_response();
 axi_slave_seq_item     pkt;
 forever begin
    `uvm_info("axi_slave_monitor :: capture_write_response","Triggred",UVM_LOW);
    pkt = axi_slave_seq_item :: type_id :: create("pkt");
    @(slave_mon_intf.slv_mon_cb); //wait till next clk posedge
    ...
    pkt.bresp  =response_t'( slave_mon_intf.slv_mon_cb.bresp);
       //`uvm_info("axi_slave_monitor :: capture_write_response","Sending pkt to SB",UVM_LOW);
       //pkt.print_write_txn(pkt);
    `uvm_info("axi_slave_monitor :: capture_write_response",$sformatf("Write pkt.operation=%s pkt.awaddr=%0d pkt.awlen=%0d, pkt.awsize=%0d, pkt.burst=%s \n pkt.wdata=%p \n pkt.wstrobe=%p",pkt.operation.name,pkt.awaddr,pkt.awlen,pkt.awsize,pkt.awburst.name,pkt.wdata,pkt.wstrobe),UVM_LOW);
       //mon_ap.write(pkt); //write pkt to sb
    //end else `uvm_error("axi_slave_monitor :: capture_write_response",$sformatf("unexpected write response, BID not found bid= %b",slave_mon_intf.slv_mon_cb.bid))
 end
endtask

task  axi_slave_monitor :: capture_read_address();
 axi_slave_seq_item     pkt;
 forever begin
    `uvm_info("axi_slave_monitor :: capture_read_address","Triggred",UVM_LOW);
    pkt = axi_slave_seq_item :: type_id :: create("pkt");
    @(slave_mon_intf.slv_mon_cb); //wait till next clk posedge
    ...
    //`uvm_info("axi_slave_monitor :: capture_read_address",$sformatf("captured address pkt put to read_address_array arid=%d",pkt.arid),UVM_LOW);
    //if(!read_address_array.exists(pkt.arid)) read_address_array[pkt.arid]=new();
    //read_address_array[pkt.arid].put(pkt);
    rdaddr.push_front(pkt.araddr);
  end
endtask

task  axi_slave_monitor ::  capture_read_data();
 axi_slave_seq_item     pkt, pkt2sb;
 //int i,no_of_beats;
 //bit last;
 forever begin
    `uvm_info("axi_slave_monitor :: capture_read_data","Triggred",UVM_LOW);
    //i = 0;
    //do begin
      pkt = axi_slave_seq_item :: type_id :: create("pkt");
      ...
      pkt.rdata_hndshk = $realtime();  //capturing timestamp for handshake.
      //pkt.rid       = slave_mon_intf.slv_mon_cb.rid;
      pkt.rdata  = slave_mon_intf.slv_mon_cb.rdata;
      pkt.rresp  = response_t'(slave_mon_intf.slv_mon_cb.rresp);
    ...
    //end while( last ==0); //keeps sampling till last indicates end of data phase.
      //pkt2sb = axi_slave_seq_item :: type_id :: create("pkt2sb");
   ...
   /*for(i=0 ; i<no_of_beats; i++)begin         //merges all beats with same rid to pkt having address info
    ...
    //`uvm_info("axi_slave_monitor :: capture_read_data","Sending pkt to SB",UVM_LOW);
    //pkt2sb.print_read_txn(pkt2sb);
    //mon_ap.write(pkt2sb); //write pkt to sb
    pkt.araddr = rdaddr.pop_back();
    if(pkt.araddr <= intc_base && pkt.araddr >= cdma_base) begin
      $display("CDMA - READ(LOAD): ADDRESS:%h --- RDATA:%h\n",pkt.araddr,pkt.rdata);
    end
    if(pkt.araddr <= 32'h8000_0000 && pkt.araddr >= intc_base) begin
      $display("INTR - READ(LOAD): ADDRESS:%h --- RDATA:%h\n",pkt.araddr,pkt.rdata);
    end
 end
endtask

task axi_slave_monitor :: merge_write_info();
axi_slave_seq_item addr_pkt,data_pkt,merged_pkt;
//int x ,no_addr, no_data; //indicates number of completed write transactions waiting for response.
  `uvm_info("axi_slave_monitor :: merge_write_info","Triggred",UVM_LOW);
  //no_addr = write_address_mbx.num();
  //no_data = write_data_mbx.num();
  //x = (no_addr<no_data)? no_addr :no_data;
  //repeat(x)begin
  forever begin
  merged_pkt = axi_slave_seq_item :: type_id :: create("merged_pkt");
  ...
  //Merging address and data phases
   ...
   //$display("awaddr = %h, wdata = %h, wstrobe = %h", merged_pkt.awaddr, merged_pkt.wdata, merged_pkt.wstrobe);
  //adding packets to array waiting for response
    //`uvm_info("axi_slave_monitor :: merge_write_info",$sformatf("putting merged pkt to wresp_array awid = %b",merged_pkt.awid),UVM_LOW);
    //if(!wresp_array.exists(merged_pkt.awid)) wresp_array[merged_pkt.awid] = new();
    //wresp_array[merged_pkt.awid].put(merged_pkt);
    if( (merged_pkt.awaddr <= intc_base) && (merged_pkt.awaddr >= cdma_base)) begin
      $display("CDMA - WRITE(STORE): ADDRESS:%h --- WDATA:%h\n",merged_pkt.awaddr-cdma_base,merged_pkt.wdata);
    end
    if(merged_pkt.awaddr <= 32'h80000000 && merged_pkt.awaddr >= intc_base) begin
      $display("INTR - WRITE(STORE): ADDRESS:%h --- WDATA:%h\n",merged_pkt.awaddr-intc_base,merged_pkt.wdata);
    end
    addr_ph_port.write(merged_pkt);
  //end
  end
endtask


task axi_slave_monitor :: capture_reset();
 axi_slave_seq_item     rst_pkt;
 `uvm_info("axi_slave_monitor :: capture_reset","Triggred",UVM_LOW);
 fork
  forever begin //reset deasserted
    @(posedge slave_mon_intf.areset_n) ;
    rst_pkt = axi_slave_seq_item :: type_id :: create("rst_pkt");
    rst_pkt.reset_op = RESET_DEASSERTED;
    rst_pkt.reset_deasserted = $realtime();
    `uvm_info("axi_slave_monitor :: capture_reset","reset_deasserted_pkt to SB",UVM_LOW);
  ...
  forever begin //reset asserted
    ...
    rst_pkt.reset_deasserted = $realtime();
    `uvm_info("axi_slave_monitor :: capture_reset","reset_asserted_pkt to SB",UVM_LOW);
    //mon_ap.write(rst_pkt);
  end
join
endtask
