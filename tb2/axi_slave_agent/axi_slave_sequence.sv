...
virtual task body ();
    ...
    forever
    begin
      bit [ADDR_WIDTH-1:0] aligned_addr, loc_addr;
      ...
      if(item.operation == WRITE)
        loc_addr = item.awaddr;
      else
        loc_addr = item.araddr;

      aligned_addr = {loc_addr[DATA_WIDTH-1:2], 2'b0};
      ...
      req.wdata = item.wdata;
      req.wstrobe = item.wstrobe;
      error_synch = 1'b1;
      ...
      if (error) begin
        ...
        req.rdata = rand_data;
        req.rresp = SLVERR;
      end else if(item.operation == READ) begin
        // Get data from memory_model, handle uninit memory accesses.
        req.rdata = read(aligned_addr, data_was_uninitialized);
      end else if(item.operation == WRITE) begin
        // Update memory_model
        write(aligned_addr, item.wdata);
